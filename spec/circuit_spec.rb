...
RSpec.describe Faulty::Circuit do
  subject(:circuit) { described_class.new(:test, cache: cache) }

  let(:open_circuit) do
    circuit = described_class.new(:test, cache: cache, rate_threshold: 0, sample_threshold: 0)
  ...
  it 'can be constructed with only a name' do
    circuit = described_class.new(:plain)
  ...
  it 'raises a CircuitTrippedError when the threshold is passed' do
    circuit = described_class.new(:test, rate_threshold: 0, sample_threshold: 0)
    expect do
      circuit.run { raise 'failed' }
    end.to raise_error(
  ...
  it 'does not close circuit until past sample threshold' do
    circuit = described_class.new(:test, rate_threshold: 0, sample_threshold: 2)
  ...
  it 'does not close circuit until past rate threshold' do
    circuit = described_class.new(:test, rate_threshold: 0.6, sample_threshold: 0)
    circuit.try_run { 'ok' }
    circuit.try_run { raise 'fail' }
    expect(circuit.status.closed?).to eq(true)
    circuit.try_run { raise 'fail' }
  ...
  it 'raises unwrapped error if error is excluded' do
    ...
    circuit = described_class.new(:test, exclude: test_error)
    expect do
  ...
  it 'raises unwrapped error if error is not included' do
    test_error = Class.new(StandardError)
    circuit = described_class.new(:test, errors: test_error)
    expect do
  ...
  it 'raises all unwrapped errors if errors option is empty' do
    circuit = described_class.new(:test, errors: [])
    expect do
